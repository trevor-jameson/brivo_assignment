...
import { fiveDayForecastTransformer } from '../utils/requestTransformers';
...
export async function getFiveDayForecast({name, lat, lon}: WeatherQueryFields) {
  try {
    const url = BASE_ONECALL_API_V3_URL + `?lat=${lat}&lon=${lon}&units=imperial&appid=${OW_API_KEY}`;

    const response = await axios.get(url);
    const transformedResponse = fiveDayForecastTransformer(response.data)
